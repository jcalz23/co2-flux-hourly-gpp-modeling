...
groups = list(data_df['site_id'].unique())

# Define experiment
exp_name = "14_2_2_5YrTrain_7DEncode_SmallNetwork_TopFeatures_NoGPP_GroupNorm"

# Experiment constants
VAL_INDEX  = 3
TEST_INDEX = 4
SUBSET_LEN = 24*365*5 # 5 year
ENCODER_LEN = 24*7  # 3 days
...
def setup_tsdataset_nogpp_slim(train_df, val_df, test_df, min_encoder_len):
    training = TimeSeriesDataSet(
      ...
      scalers={'groupnorm': GroupNormalizer(scale_by_group=True)},
...
with open(best_model_path, 'rb') as f:
    s3.upload_fileobj(f, bucket_name, best_model_path)
print("Done")